...
# --- Paths (Google Colab) ----------------------------------------------------
SCRIPT_DIR = "/content"
DATASET_DIR = "/content/dataset"        # dataset is auto-downloaded here
OUTPUT_DIR = "/content/outputs"         # charts go here (no Word report in Colab)
...
# =============================================================================
# Step 0b - Google Colab bootstrap: install packages + download the dataset
# =============================================================================
def in_colab():
...
def pip_install(packages):
    """Install pip packages quietly (used for the few not preinstalled)."""
    import subprocess
    import sys
...
def setup_kaggle_credentials():
    """Make sure ~/.kaggle/kaggle.json exists so the Kaggle API can authenticate.

    Tries, in order: Colab Secrets (KAGGLE_USERNAME / KAGGLE_KEY), an existing
    kaggle.json, then an interactive upload.
    """
    import json
    ...
    # (b) kaggle.json already present (in ~/.kaggle or the working dir)
    for cand in (kaggle_json, "/content/kaggle.json", "kaggle.json"):
        if os.path.isfile(cand):
            if cand != kaggle_json:
                import shutil
                shutil.copy(cand, kaggle_json)
            os.chmod(kaggle_json, 0o600)
            print(f"  Kaggle credentials loaded from {cand}.")
            return True

    # (c) interactive upload
    try:
        from google.colab import files
        print("  Please upload your kaggle.json (Kaggle -> Account -> API token):")
        ...
        print(f"  Could not obtain Kaggle credentials: {exc}")
    return False


def download_dataset():
    """Download + unzip the Kaggle dataset into DATASET_DIR (skip if present)."""
    if find_split_dirs(DATASET_DIR):
        print("  Dataset already present -> skipping download.")
        return True
    if not setup_kaggle_credentials():
        print("  ERROR: no Kaggle credentials -> cannot download the dataset.")
        return False
    os.makedirs(DATASET_DIR, exist_ok=True)
    import subprocess
    print(f"  Downloading '{KAGGLE_DATASET}' from Kaggle (this can take a while)...")
    subprocess.run(["kaggle", "datasets", "download", "-d", KAGGLE_DATASET,
                    "-p", DATASET_DIR, "--unzip"], check=False)
    ok = bool(find_split_dirs(DATASET_DIR))
    print("  Download complete." if ok else "  Download FAILED or layout unknown.")
    return ok


def colab_bootstrap():
    """Install missing packages and fetch the dataset (Colab only)."""
    if not in_colab():
        print("  Not running in Colab -> skipping install/download bootstrap.")
        return
    print("  Installing required packages (kaggle) ...")
    pip_install(["kaggle"])
    download_dataset()


def download_outputs():
    """Zip the outputs folder and trigger a browser download (Colab only)."""
    if not in_colab():
        return
    import shutil
    archive = shutil.make_archive("/content/exam3_outputs", "zip", OUTPUT_DIR)
    try:
        from google.colab import files
        print(f"  Downloading results archive: {os.path.basename(archive)}")
        files.download(archive)
    except Exception as exc:
        print(f"  (Could not auto-download; find the files in {OUTPUT_DIR}: {exc})")


def main():
    print("=" * 70)
    print("Face recognition with LBPH features + SVM (libsvm) - COLAB")
    print("=" * 70)

    # --- Step 0b: Colab bootstrap (install packages + download dataset) ---
    print("\n[Step 0b] Colab bootstrap ...")
    colab_bootstrap()
    ...
    # (Word report generation is intentionally skipped in the Colab version.)

    # --- Step 7b: download the charts to your computer (Colab) ---
    print("\n[Step 7b] Packaging charts for download ...")
    download_outputs()
    print("=" * 70)
